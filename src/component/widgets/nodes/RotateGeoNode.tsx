...
function RotateGeoNode(props: NodeProps<DataType>) {
...
useEffect(() => {
    if (geoData?.data && vectorData?.data) {
        ...
        if (geo) {
            ...
            if (cloned && vec) {
                cloned.rotateX(vec.x.f() ?? 0);
                cloned.rotateY(vec.y.f() ?? 0);
                cloned.rotateZ(vec.z.f() ?? 0);
                console.log("Cloned rotated:", cloned);
    ...
    <div className="border border-none rounded-lg cnode rounded-t-lg geo">
      ...
      <div className="">
        ...
        <div className="relative py-1">
          ...
          />
          <label className="label px-5">Rotate</label>
        </div>
        <div className="relative py-1">
          <Handle
            type="source"
