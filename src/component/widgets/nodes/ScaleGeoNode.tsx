...
function ScaleGeoNode(props: NodeProps<DataType>) {
  ...
  useEffect(() => {
    if(geoData?.data && vectorData?.data){
      ...
      if(geo){
          ...
          if(cloned && vec){
            cloned.scale(vec.x.f() ?? 1, vec.y.f() ?? 1, vec.z.f() ?? 1);
  ...
  return (
    <div className="border border-gray-300 rounded-lg bg-white">
      <div className="bg-slate-700 p-2 px-4 rounded-t-lg">
        <h3 className="text-sm font-semibold text-white">ScaleGeoNode</h3>
      </div>
      <div className="">
        ...
        <div className="relative py-1">
          ...
          />
          <label className="label px-5">Scale</label>
        </div>
        <div className="relative py-1">
          <Handle
            type="source"
